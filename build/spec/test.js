    ...
    describe('All test cases for Users', function () {
      ...
      describe('All test cases for user signIn ', function () {
        ...
        it('should not login a new user account and return a `422`', function (done) {
          request.get('/api/v1/auth/signin').send({
            email: '',
            password: ''
          }).expect(422).end(function (err, res) {
        ...
        it('should Login a new user and return a `201`', function (done) {
          ...
          request.get('/api/v1/auth/signin').send(userInfo).expect(201).end(function (err, res) {
            myToken = res.body.data.Token;
            expect(res.body).to.have.property('status');
            done();
          });
        });
        it('Should cancel trip `201`', function (done) {
          request.patch('/api/v1/trips/1').set('x-access-token', myToken).send({}).expect(201).end(function (err, res) {
            expect(res.body).to.have.property('message');
            done();
          });
        }); //   });
